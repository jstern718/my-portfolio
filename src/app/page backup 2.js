...
export default function Home() {
  ...
  // ── White space expansion (from line to half-screen) ──
  // During phase 0-0.3: white space expands from 0 to 50vh
  const whiteSpaceHeight = useTransform(ghostProgress, [0, 0.3], [0, '50vh']);

  // Ghost image opacity — fades in as white space opens
  const ghostImageOpacity = useTransform(ghostProgress, [0.05, 0.25], [0, 1]);

  // Top paras fade as ghost progresses (over the full range)
  const topParaOpacity = useTransform(ghostProgress, [0, 0.5, 1], [1, 0.55, 0.4]);

  // Ghost scale — desktop and mobile variants
  // Phase 0-0.3: grows from small to fill the white space
  // Phase 0.3-1: continues to scale as components build
  const ghostScaleDesktop = useTransform(ghostProgress, [0, 1], [0.4, 1]);
  const ghostScaleMobile  = useTransform(ghostProgress, [0, 1], [0.25, 0.8]);
  ...
  return (
    <div className="bg-primary">
      ...
      <div className="relative px-8 md:px-20 flex flex-col items-start lg:items-center">
        ...
        <div className={`max-w-md w-full flex flex-col gap-6 pb-6 ${hijacking ? 'invisible' : 'visible'}`}>
      ...
      <AnimatePresence>
        {hijacking && (
          <motion.div
            ...
            className="fixed inset-0 z-30 bg-primary pointer-events-none overflow-hidden"
          >
            <div className="h-screen w-screen flex flex-col px-8 md:px-20 py-10 justify-start">

              {/* TOP: Paras 1 & 2 — visible at top, fade out as progress increases */}
              <motion.div
              ...
              {/* MIDDLE: Expanding white space with ghost growing inside */}
              <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden mt-6 w-full">

                {/* Expanding white space background — grows from line to 50vh */}
                <motion.div
                  style={{ height: whiteSpaceHeight }}
                  className="w-full bg-white"
                />

                {/* Ghost illustration — positioned absolutely within and behind the white space */}
                <motion.div
                  style={{
                    opacity: ghostImageOpacity,
                    position: 'absolute',
                    top: '50%',
                    left: '50%',
                    x: '-50%',
              ...
              {/* BOTTOM: Paras 3 & 4 — appear below expanding space, clear as ghost completes */}
              <motion.div
                style={{ opacity: bottomParaOpacity, filter: bottomParaFilter }}
                className="max-w-md w-full flex flex-col gap-4 shrink-0 mt-6"
              >
